...
const SectionHeader: React.FC<SectionHeaderProps> = ({
  ...
  return (
    <div className={`flex flex-col md:flex-row gap-6 mb-12 ${center ? 'items-center justify-center text-center' : isColorful ? 'items-end justify-between text-left' : 'items-end justify-between text-left'
      }`}>
      <div className={`flex-1 ${center ? 'max-w-3xl' : 'max-w-xl'}`}>
        ...
        {title && (
          <h2 className={`${isColorful ? 'text-3xl  font-bold leading-tight' : 'text-2xl md:text-3xl font-extrabold tracking-tight leading-tight'} text-foreground`}>
            {title}
          </h2>
        )}
      ...
      {(linkText || linkHref) && (
        <Link
          href={linkHref || "#"}
          className={`flex items-center gap-2 font-bold transition-all duration-300 group whitespace-nowrap pb-2 ${isColorful ? 'text-sm hover:text-primary' : 'text-primary hover:gap-3'
            } ${center ? 'mt-4' : ''}`}
        >
          {linkText || "View all"}
          {/* {isColorful ? (
